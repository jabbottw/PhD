...
#########################   Init GUI  #########################
  ...
  def initGui(self):  
    ...
    self.action = QAction(QIcon(":/plugins/COGCC_Download/icon.png"), \
        "COGCC_DL", self.iface.mainWindow())
  ...
  def handlegetWellDataButton(self):
    ...
    if len(self.__apiList) == 0:
      ...
      for ix in range(len(self.__apiList)):
        ...
        # Determine which state the well belongs to so we can utalize the necessary download method
...
#########################   Utility Methods  #########################
  ...
  def createNewReportDialogMessage(self, newMessage):
    # Cleare the current report dialog browser                             
    self.dlg.clearReportDialogBrowser()
  ...
  def downloadCOGCC_Data(self, currentAPI):
    ...
    cleanApi = self.__DataProcessor.cleanCO_API(currentAPI)
    ...
    if self.__phd_Update == False:
      dlStatus = self.__WellDataCollector.Download_COGCC_Data(cleanApi[2:10], self.__COGCC_fClass, self.__folder)
    ...
    # Otherwise, check the database to see if the current well has already been processed. If it hasn't, then process the current well. 
      ...
      dbList = self.__DataProcessor.getDBData("select * from colorado.co_whf_dl_status where co_whf_dl_status.api = '%s' AND well_proce = 'Completed'" % (cleanApi))
      if not dbList:
  ...
  def downloadUtah_Data(self, currentAPI):
    ...
    dbList = self.__DataProcessor.getDBData("select * from utah.ut_whf_dl_status where api_10 = '%s' AND well_proce = 'True'" %(currentAPI))
    ...
    if not dbList:
      ...
      dlStatus = self.__WellDataCollector.Download_Utah_Data(currentAPI, self.__folder)
      # Update the current well within the database
      # Figure out if the current well exists in the DB
      wellExists = self.__DataProcessor.getDBData("select * from utah.ut_whf_dl_status where api_10 = '%s'" %(currentAPI))
      ...
      if wellExists:
        ...
        self.__DataProcessor.inputDBData(sqlInput)
        sqlInput = ""
